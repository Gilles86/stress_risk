...
% 
%
% ========= adapted to allow only a certain amount of characters ===========
% - maximum set on line 140: "max_length"
% - if too many characters entered, will display "(Maximum number of characters reached.)"
% ==========================================================================
%
    ...
    switch abs(char)
        case {13, 3, 10, 27}
            ...
            if ~isempty(string)
                ...
                Screen('DrawText', windowPtr, '(Please enter some text.)', x+350, y+200, textColor, bgColor);
            end
        case 8
            % backspace
            if ~isempty(string)
...
%     if numel(string) > max_length
%         string_elements = strsplit(string,'  ');
%         if numel(string_elements{end}) > max_length
%             string_elements{end} = [string_elements{end}(1:max_length) '\n  ' string_elements{end}(max_length+1:end)]
%         end
%         string = strjoin(string_elements);
%     end

    if numel(string) > max_length
        string = string(1:max_length);
        Screen('DrawText', windowPtr, '(Maximum number of characters reached.)', x+300, y+200, textColor, bgColor);
    end
