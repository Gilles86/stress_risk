function first_level_spm_Vchoice3(subject,session,bids_folder, model)
   ...
   for run= 1:6
    ...
    matlabbatch{2}.spm.stats.fmri_spec.sess(run).scans = cellstr(spm_file(ff));
            ...
            matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).duration = v1_unchosen(:,2);
                matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).pmod.name = 'v1_unchosen';
                matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).pmod.param = v1_unchosen(:,3);
