function first_level_spm_EV(subject,session,bids_folder)
   ...
   clear matlabbatch
    ...
    matlabbatch{1}.cfg_basicio.file_dir.dir_ops.cfg_mkdir.name = '1stlevel_EV';
    %--------------------------------------------------------------------------
    matlabbatch{2}.spm.stats.fmri_spec.dir = cellstr(fullfile(target_folder,'1stlevel_EV'));
   ...
   for run= 1:6
    ...
    onsets_n1 = load(strcat(target_folder,'/',subject,'_',session,'_task-risk_run-',num2str(run),'_events_n1-EV.txt'));
    onsets_n2 = load(strcat(target_folder,'/',subject,'_',session,'_task-risk_run-',num2str(run),'_events_n2-EV.txt'));
    ...
    matlabbatch{3}.spm.stats.fmri_est.spmmat = cellstr(fullfile(target_folder,'1stlevel_EV','SPM.mat'));
    ...
    matlabbatch{4}.spm.stats.con.spmmat = cellstr(fullfile(target_folder,'1stlevel_EV','SPM.mat'));
    matlabbatch{4}.spm.stats.con.consess{1}.tcon.name = 'n1_EV';
    matlabbatch{4}.spm.stats.con.consess{1}.tcon.weights = n1;
    matlabbatch{4}.spm.stats.con.consess{2}.tcon.name = 'n2_EV';
