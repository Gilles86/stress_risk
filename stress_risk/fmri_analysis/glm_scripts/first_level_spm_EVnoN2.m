function first_level_spm_EVnoN2(subject,session,bids_folder, model)
   
   model_name = ['1stlevel_' model 'noN2']
   ...
   for run= 1:6
    ...
    onsets_n1 = load(strcat(target_folder,'/',subject,'_',session,'_task-risk_run-',num2str(run),['_events_n1-' model '.txt']));
    %onsets_n2 = load(strcat(target_folder,'/',subject,'_',session,'_task-risk_run-',num2str(run),['_events_n2-' model '.txt']));
    onsets_diff = load(strcat(target_folder,'/',subject,'_',session,'_task-risk_run-',num2str(run),['_events_diff-' model '.txt']));
    ...
    matlabbatch{2}.spm.stats.fmri_spec.sess(run).scans = cellstr(spm_file(ff));
            ...
            matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).onset = onsets_diff(:,1);    % vetor like data
            matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).duration = onsets_diff(:,2); %reaction times
            matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).pmod.name = 'diff';
            matlabbatch{2}.spm.stats.fmri_spec.sess(run).cond(2).pmod.param = onsets_diff(:,3);
    ...
    load(file_nuisance);
    n_cond_reg = 4 % 2 von n1, 3 from n2 (cond, value, diff-choice)
    n_conf = size(R,2);
