...
for n=1:3
    ...
    out_folder= sprintf(['2ndlevel_' model specification '_%s'], num)
    ...
    for sub=1:numel(subList)
         for ses = 1:length(sesList)
            con_file = fullfile(bids_folder,'derivatives','spm',subList{sub},sesList{ses},in_model_name,sprintf('con_000%s.nii',num));
            if isfile(con_file)
