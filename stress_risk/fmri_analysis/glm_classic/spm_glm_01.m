...
data_json = jsondecode(fileread('/Users/mrenke/data/ds-stressrisk/task-risk_bold.json'))
...
for sub = 1:numel(subList)
    for ses = 1:length(sesList)
       try
           data_path = strcat(bids_folder,'/derivatives/fmriprep/',sub,'/',ses,'/func/');
           ...
           for run= 1:6
            ...
            matlabbatch{2}.spm.stats.fmri_spec.sess(1).scans = cellstr(spm_file(ff));
    
                    % Conditions --> columns in design matrix
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).name = 'n1';
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).onset = onsets_n1(:,1);    % vetor like data
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).duration = 0.6;
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).pmod.name = 'value';
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).pmod.param = onsets_n1(:,3);
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).pmod.poly = 1;
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(1).orth = 1; % if additional pmod for same cond. set to 0
                    
    
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).name = 'n2';
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).onset = onsets_n2(:,1);    % vetor like data
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).duration = 0.6;
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).pmod.name = 'value';
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).pmod.param = onsets_n2(:,3);
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).pmod.poly = 1;
                    matlabbatch{2}.spm.stats.fmri_spec.sess(1).cond(2).orth = 1;

            matlabbatch{2}.spm.stats.fmri_spec.sess(1).multi_reg   = rp_file_1; %cellstr(spm_file(rp_file_1));
