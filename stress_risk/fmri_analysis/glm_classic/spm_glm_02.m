    ...
    for ses = 1:length(sesList)
       try
           ...
           for run= 1:6
            ...
            im_file = dir(strcat(data_path,'*run-', num2str(run),pattern, '*.nii.gz'));
    ...
    for ses = 1:length(sesList)
       try
           ...
           for run= 1:6
            ...
            strcat(subList{sub}, ' ', num2str(run), 'done') % print
            end
       end
    end
end
...
for run=1:6
    ...
    physioR_file = (fullfile(bids_folder,'derivatives','physiotoolbox', subList{sub}, sesList{ses},'func', [subList{sub} '_' sesList{ses} '_task-task_run-' num2str(run) '_desc-retroicor_output.mat']));
        ...
        %
        file_nuisance = fullfile(bids_folder,'derivatives','spm', subList{sub}, sesList{ses},[subList{sub} '_' sesList{ses} '_task-risk_run-' num2str(run) '_mult-reg-confounds.mat'])
    ...
    for ses = 1:length(sesList)
       try
           ...
           for run= 1:6
            ...
            onsets_n1 = load(strcat(target_folder,'/',subList{sub},'_',sesList{ses},'_task-risk_run-',num2str(run),'_events_n1.txt'));
            onsets_n2 = load(strcat(target_folder,'/',subList{sub},'_',sesList{ses},'_task-risk_run-',num2str(run),'_events_n2.txt'));
