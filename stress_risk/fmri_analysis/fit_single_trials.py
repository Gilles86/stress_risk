...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 17:01:31 2022
...
def main(subject, session, bids_folder, smoothed=True, 
    ...
    ims = [op.join(derivatives, 'fmriprep', f'sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-{run}_space-T1w_desc-preproc_bold.nii.gz') for run in runs]
    #ims = [op.join(derivatives, f'sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-{run}_space-T1w_desc-preproc_bold.nii.gz') for run in runs]

    mask = op.join(derivatives, 'fmriprep', f'sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-1_space-T1w_desc-brain_mask.nii.gz')
    #mask = op.join(derivatives, f'sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-1_space-T1w_desc-brain_mask.nii.gz')
    ...
    for run in runs:
        behavior.append(pd.read_table(op.join(
            bids_folder, f'sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-{run}_events.tsv')))
    ...
    # # sub-02_ses-7t2_task-risk_run-1_space-fsaverage_hemi-R_bold.func
    ...
    fmriprep_confounds = [
        op.join(bids_folder, 'derivatives', 'fmriprep', f'sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-{run}_desc-confounds_timeseries.tsv') for run in runs] #changed timesereis to regressors
    fmriprep_confounds = [pd.read_table(
        cf)[fmriprep_confounds_include] for cf in fmriprep_confounds]

    retroicor_confounds = [
        op.join(bids_folder, f'derivatives/physiotoolbox/sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-risk_run-{run}_desc-retroicor_timeseries.tsv') for run in runs]
    ...
    single_trial_betas.to_filename(op.join(base_dir, f'sub-{subject}_ses-{session}_task-risk_space-T1w_desc-stims1_pe.nii.gz'))
...
if __name__ == '__main__':
 ...
 # things changed: 
     #replaced task-task with task-risk   
     #line 102
